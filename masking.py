...
def process_file_local(file_url, text_to_detect, output_dir='output'):
    """
    Process a file locally (without Lambda) and save to local directory.
    
    Args:
        file_url: URL of the file to process
        text_to_detect: Text to mask in the file
    ...
    try:
        ...
        if file_type == 'image':
            ...
            if ext == 'jpeg':
                ext = 'jpg'
            output_path = os.path.join(output_dir, f"{url_hash}_processed.{ext}")
            ...
            # Save to local file
            url_hash = hashlib.md5(file_url.encode()).hexdigest()
            output_path = os.path.join(output_dir, f"{url_hash}_processed.pdf")
            
            with open(output_path, 'wb') as f:
                f.write(processed_file)
            
            print(f"Saved processed PDF: {output_path}")
        
        log_timing("Total processing time", total_start_time)
        
        # Return absolute path and URL-friendly path
        abs_path = os.path.abspath(output_path)
        return {
            'output_path': abs_path,
            'output_url': f"file://{abs_path}",
...
def main():
    """Local runner that processes files from table.csv"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Process files from CSV for masking')
    parser.add_argument('--csv', type=str, default='table.csv', help='CSV file path')
    parser.add_argument('--row', type=int, help='Process specific row number (1-based)')
    ...
    with open(args.csv, 'r') as f:
        ...
        for row_num, row in enumerate(reader, start=1):
            ...
            url = row[1].strip()
            target_text = row[2].strip() if len(row) > 2 else ""
            
            # Skip empty URLs
            if not url or url.startswith('P:\\'):
                continue
    ...
    for idx, row_info in enumerate(rows_to_process, 1):
        ...
        url = row_info['url']
        target_text = row_info['target_text']
        file_type = row_info['type']
        
        print("\n" + "="*80)
        print(f"Processing {idx}/{len(rows_to_process)}: Row {row_num} ({file_type})")
        print(f"URL: {url}")
        ...
        try:
            ...
            if args.skip_existing:
                ...
                file_type_detected, file_format = detect_file_type(url)
                if file_type_detected == 'image':
                    ext = file_format.lower()
                    if ext == 'jpeg':
                        ext = 'jpg'
                    output_path = os.path.join(args.output, f"{url_hash}_processed.{ext}")
                else:
                    output_path = os.path.join(args.output, f"{url_hash}_processed.pdf")
                
                if os.path.exists(output_path):
                    abs_path = os.path.abspath(output_path)
                    print(f"⚠ Output already exists, skipping: {abs_path}")
                ...
                # By default, overwrite existing files (reprocess)
                url_hash = hashlib.md5(url.encode()).hexdigest()
                file_type_detected, file_format = detect_file_type(url)
                if file_type_detected == 'image':
                    ext = file_format.lower()
                ...
                if os.path.exists(output_path):
                    print(f"⚠ Output file exists, will be overwritten: {output_path}")
            
            # Process the file
            result = process_file_local(url, target_text, args.output)
    ...
    if results['success']:
        ...
        for item in results['success']:
            status = " (skipped)" if item.get('skipped') else ""
            print(f"  - Row {item['row']}: {item['output_url']}{status}")
    
    if results['failed']:
        print("\n✗ Failed to process:")
        for item in results['failed']:
            print(f"  - Row {item['row']}: {item['url']}")
